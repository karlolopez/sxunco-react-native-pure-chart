...
export const initData = (dataProp, height, gap, numberOfPoints = 5, minYValue, maxYValue) => {
  ...
  sortedData = refineData(dataProp, max, height, gap, maxYValue, minYValue)
  return {
  ...
  flattenData.map((series) => {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (typeof dataProp[i] === 'number') {
        ...
        */
        dataObject.ratioY = (dataProp[i]-min) / (maxClone-min) * height
        dataObject.y = dataProp[i]
        dataObject = {
          gap: i * gap,
          // original ratioY: dataProp[i] / maxClone * height,
          ratioY:(dataProp[i]-min) / (maxClone-min) * height,
          y: dataProp[i]
        }
        ...
        if (typeof dataProp[i].y === 'number' && dataProp[i].x) {
          ...
          dataObject = {
            gap: i * gap,
            //ORiginal ratioY: dataProp[i].y / maxClone * height,
            ratioY: (dataProp[i].y -min) / (maxClone-min) * height,
            x: dataProp[i].x,
            y: dataProp[i].y,
            isEmpty: isEmpty
          }
...
export const getGuideArray = (max, height, numberOfPoints = 5,  maxYValue, minYValue) => {
  
  let x = parseInt(max)
  let definedMax = parseInt(max)
  ...
  let incr = (definedMax - min) / numberOfPoints
  for (let i = 1; i < numberOfPoints + 1; i++) {    
    let  val = min + (i*incr)
    arr.push([val + postfix, height / numberOfPoints * i, 1 * temp / max * height])		
  }
